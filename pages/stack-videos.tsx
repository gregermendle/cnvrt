...
import { stackVideos } from "../lib/video";

export const StackVideos: NextPageWithLayout = () => {
  const [stackVideo, stackStatus, stackProgress] = useFFmpeg(stackVideos);

  async function stack(files: [File, File]) {
    const mp4 = await stackVideo(files);
    if (mp4) {
  ...
  return (
    <div className="h-full max-h-full px-2">
      <PageMeta />
      <DragAndDrop allowedTypes={["video"]} multiple>
          ...
          <>
            <Card className="w-full max-w-2xl mx-auto md:mt-16 mt-8">
              ...
              <div className="px-6 pb-6 flex flex-col items-start justify-start gap-4">
                ...
                {files.length === 2 && (
                  <Button onClick={() => stack([files[0], files[1]])}>
  ...
  return (
    <div className="w-full">
      <div className="ring ring-primary-600 rounded-md overflow-hidden">
        ...
        <canvas ref={canvas} className="object-cover h-72 w-full" />
      </div>
    </div>
  );
};
